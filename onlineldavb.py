...
def parse_doc_list(docs, vocab):
    ...
    for d in range(0, D):
        ...
        for word in words:
            if (word in vocab):
                wordtoken = vocab[word] # changed it usingi ndex
...
class OnlineLDA_sparse:
    """
    Implements online VB for LDA as described in (Hoffman et al. 2010).
    """

    def __init__(self, vocab, K, D, alpha, eta, tau0, kappa,sparse=True):
        """
        Arguments:
        K: Number of topics
        ...
        self._N_tilde = self._lambda - self._eta # initialize N_tilde/pi
    ...
    def do_e_step(self, wordids, wordcts):
        ...
        for d in range(0, batchD):
            ...
            ids = list(wordids[d]) # 改动： change to list
            cts = list(wordcts[d]) # 改动： change to list
            gammad = gamma[d, :]
            ...
            # 加入 sparsity
            # add spasity in sparse stochastic
            # Note N_d is the lenght of document d
...
#         return((gamma, sstats))
    ...
    def update_lambda_docs(self, docs):
        ...
        # Use sparse update.
        if self.sparse == True:
            self._N_tilde = self._N_tilde * (1 - rhot) + rhot * self._D * sstats / len(docs) 
            self._lambda = self._N_tilde + self._eta
...
#             score += n.sum(cts * n.log(phinorm))
    ...
    def approx_bound_docs(self, docs, gamma):
        ...
        for d in range(0, batchD):
            gammad = gamma[d, :]
            ids = list(wordids[d]) # 改动list
            cts = list(wordcts[d]) # 原来 np.array
            phinorm = n.zeros(len(ids))
            for i in range(0, len(ids)):
                temp = Elogtheta[d, :] + self._Elogbeta[:, ids[i]]
                tmax = max(temp)
...
#             score += n.sum(cts * n.log(phinorm))
    ...
    def approx_bound_docs_lambda(self, docs, gamma,_lambda):
        """
        Estimates the variational bound over *all documents* using only
        the documents passed in as "docs." gamma is the set of parameters
        to the variational distribution q(theta) corresponding to the
...
#             score += n.sum(cts * n.log(phinorm))
        ...
        # E[log p(beta | eta) - log q (beta | lambda)]
        score = score + n.sum((self._eta-_lambda)*self._Elogbeta)
        score = score + n.sum(gammaln(_lambda) - gammaln(self._eta))
        score = score + n.sum(gammaln(self._eta*self._W) - 
                              gammaln(n.sum(_lambda, 1)))
